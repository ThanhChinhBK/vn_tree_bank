    ...
    def __init__(
      ...
      char_embedd_dim = 30, n_hidden_LSTM =200,max_char_per_word=45):
        ...
        with tf.device('/cpu:0'), tf.name_scope("char_embedding"):
            ...
            self.W_char = tf.Variable(tf.random_uniform([char_vocab_size+1, char_embedd_dim],-1,1),
                                      trainable=True, name="W_char")
            ...
            #shape [batch_size,max_char_per_word*sequence_length,char_embedd_dim]
            self.embedded_char_dropout =tf.nn.dropout(self.embedded_char, 
                                                      self.dropout_keep_prob,name="embedded_char_dropout")
        ...
        with tf.name_scope("char_conv_maxPool"):
            ...
            self.word_char_features_dropout =tf.nn.dropout(self.word_char_features, 
                                                           self.dropout_keep_prob,name="word_char_features_dropout")
        ...
        with tf.name_scope("output"):
            ...
            self.biLstm_reshaped = tf.reshape(self.biLstm_dropout, [-1, 2*n_hidden_LSTM]) 
            # [batch_size * timesteps , 2*n_hidden_LSTM] obtained by statement print(self.biLstm.get_shape())

            # Final (unnormalized) scores and predictions
            self.predictions = tf.nn.xw_plus_b(self.biLstm_reshaped, W_out, b_out, name="predictions") 
            # input : [batch_size * timesteps , 2*n_hidden_LSTM] * [2*n_hidden_LSTM, num_classes]  
